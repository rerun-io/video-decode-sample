...
/**
  ...
  @param {ArrayBuffer | null | undefined} arrayBuffer ArrayBuffer to read from.
  @param {Number | null | undefined} byteOffset Offset from arrayBuffer beginning for the DataStream.
  @param {Boolean | null | undefined} endianness DataStream.BIG_ENDIAN or DataStream.LITTLE_ENDIAN (the default).
  */
...
MultiBufferStream.prototype.logBufferLevel = function (info) {
  ...
  var log = info ? console.info : console.debug;
...
MultiBufferStream.prototype.cleanBuffers = function () {
  ...
  for (i = 0; i < this.buffers.length; i++) {
    ...
    if (buffer.usedBytes === buffer.byteLength) {
      console.debug("MultiBufferStream", "Removing buffer #" + i);
      this.buffers.splice(i, 1);
      i--;
    }
  }
...
BoxParser.parseOneBox = function (stream, headerOnly, parentSize) {
  ...
  if (stream.getEndPosition() - start < 8) {
    console.debug("BoxParser", "Not enough data in stream to parse the type and size of the box");
    return { code: BoxParser.ERR_NOT_ENOUGH_DATA };
  }
  if (parentSize && parentSize < 8) {
    console.debug("BoxParser", "Not enough bytes left in the parent box to parse a new box");
    return { code: BoxParser.ERR_NOT_ENOUGH_DATA };
  }
  ...
  if (type == "uuid") {
    if (stream.getEndPosition() - stream.getPosition() < 16 || parentSize - hdr_size < 16) {
      stream.seek(start);
      console.debug("BoxParser", "Not enough bytes left in the parent box to parse a UUID box");
      return { code: BoxParser.ERR_NOT_ENOUGH_DATA };
    }
...
BoxParser.createBoxCtor("av1C", function (stream) {
  ...
  if ((tmp >> 7) & (0x1 !== 1)) {
    console.error("av1C marker problem");
    return;
  }
  this.version = tmp & 0x7f;
  if (this.version !== 1) {
    console.error("av1C version " + this.version + " not supported");
    return;
  }
  ...
  if (this.reserved_1 !== 0) {
    console.error("av1C reserved_1 parsing problem");
    return;
  }
  ...
  if (this.initial_presentation_delay_present === 1) {
    ...
    if (this.reserved_2 !== 0) {
      console.error("av1C reserved_2 parsing problem");
      return;
    }
  }
...
BoxParser.createFullBoxCtor("ctts", function (stream) {
  ...
  if (this.version === 0) {
    for (i = 0; i < entry_count; i++) {
      ...
      if (value < 0) {
        console.warn("BoxParser", "ctts box uses negative values without using version 1");
      }
...
BoxParser.createFullBoxCtor("emsg", function (stream) {
  ...
  var message_size =
    this.size - this.hdr_size - (4 * 4 + (this.scheme_id_uri.length + 1) + (this.value.length + 1));
  if (this.version == 1) {
    message_size -= 4;
  }
  ...
  for (var i = 0; i < this.entry_count; i++) {
    ...
    if (ret.code === BoxParser.OK) {
      if (ret.box.type !== "infe") {
        console.error("BoxParser", "Expected 'infe' box, got " + ret.box.type);
      }
...
BoxParser.createFullBoxCtor("infe", function (stream) {
  ...
  if (this.version === 1) {
    ...
    console.warn("BoxParser", "Cannot parse extension type");
    stream.seek(this.start + this.size);
    return;
  }
  if (this.version >= 2) {
...
BoxParser.createFullBoxCtor("mehd", function (stream) {
  if (this.flags & 0x1) {
    console.warn("BoxParser", "mehd box incorrectly uses flags set to 1, converting version to 1");
    this.version = 1;
  }
  if (this.version == 1) {
...
BoxParser.createSampleGroupCtor("dtrt", function (stream) {
  console.warn("BoxParser", "Sample Group type: " + this.grouping_type + " not fully parsed");
});
...
BoxParser.createSampleGroupCtor("rash", function (stream) {
  ...
  if (
    ...
    console.warn("BoxParser", "Mismatch in " + this.grouping_type + " sample group length");
...
BoxParser.SampleGroupEntry.prototype.parse = function (stream) {
  console.warn("BoxParser", "Unknown Sample Group type: " + this.grouping_type);
...
BoxParser.createFullBoxCtor("sgpd", function (stream) {
  this.grouping_type = stream.readString(4);
  console.debug("BoxParser", "Found Sample Groups of type " + this.grouping_type);
  if (this.version === 1) {
  ...
  for (i = 1; i <= entryCount; i++) {
    ...
    if (ret.code === BoxParser.OK) {
      if (BoxParser[ret.type + "SampleEntry"]) {
        ...
        console.warn("BoxParser", "Unknown sample entry type: " + ret.type);
...
BoxParser.createFullBoxCtor("stz2", function (stream) {
  ...
  if (this.version === 0) {
    ...
    if (this.field_size === 4) {
      ...
      console.error("BoxParser", "Error in length field in stz2 box");
    }
  }
});
...
BoxParser.ContainerBox.prototype.write = function (stream) {
  ...
  console.debug("BoxWriter", "Adjusting box " + this.type + " with new size " + this.size);
...
ISOFile.prototype.flush = function () {
  console.info("ISOFile", "Flushing remaining samples");
  this.updateSampleLists();
...
ISOFile.prototype.seekTrack = function (time, useRap, trak) {
  ...
  if (trak.samples.length === 0) {
    console.info(
      "ISOFile",
      "No sample in track, cannot seek! Using time " + getDurationString(0, 1) + " and offset: " + 0
    );
  ...
  console.info(
    ...
    "Seeking to " +
      ...
      trak.tkhd.track_id +
      ", time " +
      getDurationString(time, timescale) +
      " and offset: " +
      seek_offset
  );
...
ISOFile.prototype.seek = function (time, useRap) {
  ...
  if (!this.moov) {
    ...
    console.info(
      "ISOFile",
      "Seeking at time " +
        getDurationString(seek_info.time, 1) +
...
ISOFile.prototype.processIncompleteMdat = function () {
  ...
  found = this.stream.seek(box.start + box.size, false, this.discardMdatData);
  if (found) {
    console.debug("ISOFile", "Found 'mdat' end in buffered data");
...
ISOFile.prototype.buildTrakSampleLists = function (trak) {
  ...
  for (j = 0; j < stsz.sample_sizes.length; j++) {
    ...
    sample.offset =
      stco.chunk_offsets[sample.chunk_index - 1] + offset_in_chunk; /* chunk indexes are 1-based */
  ...
  if (meta.iloc) {
    for (i = 0; i < meta.iloc.items.length; i++) {
      ...
      if (itemloc.data_reference_index !== 0) {
        console.warn("Item storage with reference to other files: not supported");
      ...
      switch (itemloc.construction_method) {
        ...
        case 1: // offset into the idat box of this meta box
          console.warn("Item storage with construction_method : not supported");
          break;
...
ISOFile.writeInitializationSegment = function (ftyp, moov, total_duration, sample_duration) {
  ...
  var trex;
  var box;
  console.debug("ISOFile", "Generating initialization segment");
...
ISOFile.prototype.initializeSegmentation = function () {
  ...
  if (this.onSegment === null) {
    console.warn("MP4Box", "No segmentation callback set!");
  }
...
var createFile = function (_keepMdatData, _stream) {
